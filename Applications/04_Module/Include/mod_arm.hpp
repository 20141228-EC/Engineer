...
#define ARM_PITCH1_PHYSICAL_RANGE_MAX 104.0f
...
#define ARM_END_PITCH_PHYSICAL_RANGE_MIN -127.7f   //145.0f
...
/*-------------------------------------临时----------------------------------------------------------*/
#define ARM_END_LENGTH_MOTOR_RANGE 81469.0f
#define ARM_END_LENGTH_MOTOR_OFFSET 0.0f
#define ARM_END_LENGTH_MOTOR_RATIO 0.0f
#define ARM_END_LENGTH_INIT_LENGTH 100.0f
...
public:
	...
	struct SModInitParam_Arm: public SModInitParam_Base {
		...
		EDeviceID MotorID_length_L = EDeviceID::DEV_NULL;  // 夹爪电机
		// EDeviceID MotorID_length_R = EDeviceID::DEV_NULL;
		...
		CInfCAN::CCanTxNode *MotorTxNode_length_L;
		// CInfCAN::CCanTxNode *MotorTxNode_length_R;
		...
		CAlgoPid::SAlgoInitParam_Pid LengthPosPidParam;
		CAlgoPid::SAlgoInitParam_Pid LengthSpdPidParam;
	...
	struct SArmInfo {
		...
		float_t angle_end_length = 0.0f; ///< 机械臂末端夹爪伸缩长度
		...
		bool isAngleArrived_End_Length = false; ///< 机械臂末端伸缩长度是否到达
	} armInfo;
	...
	struct SArmCmd {
		...
		float_t set_angle_end_length = 0.0f; ///< 机械臂末端伸缩长度设定
	} armCmd;
...
private:
	...
	class CComClaw: public CComponentBase {
	public:
		enum
		{
			L = 0,
		...
		const int32_t rangeLimit_Length = ARM_END_LENGTH_MOTOR_RANGE; ///< 电机位置范围限制

		// 定义机械臂末端信息结构体并实例化
		struct SClawInfo
		{
			int32_t posit_Length = 0;		   ///< Claw Posit Length
			bool isPositArrived_Length = false; ///< Claw Posit Arrived Length
		} clawInfo;

		// 定义机械臂末端控制命令结构体并实例化
		struct SClawCmd
		{
			int32_t setPosit_Length = 0; ///< Claw Posit Set Length
		} clawCmd;

		// 电机实例指针数组
		std::array<CDevMtr *, 1> motor = {nullptr};
		...
		// 物理位置转换为电机位置: Length
		static int32_t PhyPositToMtrPosit_Length(float_t phyPosit);

		// 电机位置转换为物理位置: Length
		static float_t MtrPositToPhyPosit_Length(int32_t mtrPosit);

		// 输出更新函数
		EAppStatus _UpdateOutput(float_t posit_Length);

		// 电机can发送节点
		std::array<CInfCAN::CCanTxNode *, 1> mtrCanTxNode;
