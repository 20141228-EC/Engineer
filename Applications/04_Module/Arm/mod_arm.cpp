...
void CModArm::UpdateHandler_() {
	...
	armInfo.angle_end_length =
		comClaw_.MtrPositToPhyPosit_Length(comClaw_.clawInfo.posit_Length);
	...
	armInfo.isAngleArrived_End_Length = comClaw_.clawInfo.isPositArrived_Length;
	...
	CDevMtrDJI::FillCanTxBuffer(comClaw_.motor[CComClaw::L],
								comClaw_.mtrCanTxNode[CComClaw::L]->dataBuffer,
								comClaw_.mtrOutputBuffer[CComClaw::L]);
	// CDevMtrDJI::FillCanTxBuffer(comClaw_.motor[CComClaw::R],
	// 							comClaw_.mtrCanTxNode[CComClaw::R]->dataBuffer,
	// 							comClaw_.mtrOutputBuffer[CComClaw::R]);
...
EAppStatus CModArm::RestrictArmCommand_() {
	...
	armCmd.set_angle_end_roll = 0.0f; // 夹爪不允许转动		
	armCmd.set_angle_end_length = 
		std::clamp(armCmd.set_angle_end_length,
				   0.0f, 100.0f);
