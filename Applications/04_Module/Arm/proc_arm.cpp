	...
	while (true) {
		...
		switch (arm.Module_FSMFlag_) {
			...
			case FSM_INIT: {
				...
				arm.comClaw_.StartComponent();
				proc_waitUntil(arm.comClaw_.componentStatus == APP_OK);
				...
				arm.armCmd.set_angle_end_length = ARM_END_LENGTH_INIT_LENGTH;
			...
			case FSM_CTRL: {
				...
				arm.comClaw_.clawCmd.setPosit_Length =
					CComClaw::PhyPositToMtrPosit_Length(arm.armCmd.set_angle_end_length);
