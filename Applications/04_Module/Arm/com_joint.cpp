...
EAppStatus CModArm::CComJoint::UpdateComponent() {
	...
	jointInfo.posit_pitch3 = motor[P3]->motorData[CDevMtr::DATA_POSIT] * ARM_PITCH3_MOTOR_DIR;		///<将电机的机械角度更新到关节类中
	...
	jointInfo.isPositArrived_pitch3 = abs(jointInfo.posit_pitch3 - jointCmd.setPosit_pitch3) < 700; ///<要求机械臂每一次运动到要在目标位置的限制范围内才能够进行下一步的动作
	...
	switch (Component_FSMFlag_) {
		...
		case FSM_PREINIT: {
			if (isreset_flag == false){
				...
				motor[P1]->motorData[CDevMtr::DATA_POSIT] = motor[P1]->motorData[CDevMtr::DATA_ANGLE] - POSIT_JOINT2_PITCH1_MACH;		///< 刚上电的时候获取初始值.距离机械限位的编码器差
				...
				if(jointInfo.isPositArrived_pitch3 && jointInfo.isPositArrived_pitch2 && jointInfo.isPositArrived_pitch1 && alreadySetYaw == true){
					if(abs(jointInfo.posit_yaw - jointCmd.setPosit_yaw)<500){									///<包含了堵转和未堵转两种标定yaw零点的情况
						...
						jointCmd.setPosit_pitch3 = POSIT_JOINT4_PITCH3_INIT_PHY * 182.04f;						///<初始化完成之后，yaw归位，p1,p2,p3抬升到一定的角度
		...
		case FSM_INIT: {
			if (jointInfo.isPositArrived_yaw && 
				jointInfo.isPositArrived_pitch1 && 
				jointInfo.isPositArrived_pitch2 && jointInfo.isPositArrived_pitch3) {			///<如果到了目标的位置
				jointCmd.setPosit_yaw = 0;
				jointCmd.setPosit_pitch1 = POSIT_JOINT2_PITCH1_INIT_PHY * 182.04f;
				jointCmd.setPosit_pitch2 = POSIT_JOINT3_PITCH2_INIT_PHY * 182.04f;
				jointCmd.setPosit_pitch3= POSIT_JOINT4_PITCH3_INIT_PHY * 182.04f;									   			///<这里似乎重复可删去
				pidPosCtrl_yaw.ResetPidController();																			
				pidSpdCtrl_yaw.ResetPidController();
				pidPosCtrl_pitch1.ResetPidController();
				pidSpdCtrl_pitch1.ResetPidController();
