	...
	switch (Component_FSMFlag_) {
		...
		case FSM_PREINIT: {
			if (isreset_flag == false){
				...
				// motor[Y]->motorData[CDevMtr::DATA_POSIT]  = motor[Y]->motorData[CDevMtr::DATA_ANGLE] * ARM_YAW_MOTOR_DIR - POSIT_JOINT1_YAW_MACH;
				// while(motor[Y]->motorData[CDevMtr::DATA_POSIT] < -32767)
				// 	motor[Y]->motorData[CDevMtr::DATA_POSIT] += 65535;//归位到-32767~32768范围内
