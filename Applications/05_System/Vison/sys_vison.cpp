...
EAppStatus CSystemVision::InitSystem(SSystemInitParam_Base *param) {
	...
	auto it = DeviceIDMap.find(visionParam.visionDevID);
	if (it != DeviceIDMap.end() && it->second != nullptr) {
		pVision_ = static_cast<CDevVision *>(it->second);
	}
...
//   }
	...
	(SystemCore.currentAutoCtrlProcess_ == CSystemCore::EAutoCtrlProcess::EXCHANGE_ORE) || (SystemCore.use_Controller_ == true) ? 1 : 0;
