...
void CSystemCore::StartPopOreTask(void *arg) {
	...
	while (keyboard.key_Ctrl) {
		...
		if (keyboard.mouse_L) {
			...
			core.parm_->armCmd.set_angle_end_pitch = -109.865f;
			...
			proc_waitUntilWithTimeout(
				...
				core.parm_->armInfo.isAngleArrived_Pitch2 == true &&
				core.parm_->armInfo.isAngleArrived_End_Pitch == true), 800
			);
		...
		if (keyboard.mouse_R) {
			...
			core.parm_->armCmd.set_angle_end_pitch = -100.970f;
			...
			// core.psubgantry_->subGantryCmd.setStretchPosit_R = 0.0f;
			// proc_waitUntilWithTimeout(
			// 	core.psubgantry_->subGantryInfo.isPositArrived_Stretch_R == true, 800
			// );
			...
			core.parm_->armCmd.set_angle_Roll = 0.0f;
			core.parm_->armCmd.set_angle_end_pitch = 0.0f;
			core.parm_->armCmd.set_angle_end_roll += 15.08f;
