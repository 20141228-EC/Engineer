...
#define RTT_CHANNEL_DEBUG               0       // 调试通道
#define RTT_CHANNEL_DATA                1       // 数据通道

/* RTT缓冲区大小配置 - 使用应用层定义 */
#define RTT_BUFFER_SIZE_UP_DEBUG       1024    // 上行通道0缓冲区大小 (调试)
#define RTT_BUFFER_SIZE_DOWN_DEBUG      32      // 下行通道0缓冲区大小
#define RTT_BUFFER_SIZE_UP_Data        512     // 上行通道1缓冲区大小 (数据)
#define RTT_BUFFER_SIZE_DOWN_Data      32      // 下行通道1缓冲区大小

// 应用层RTT通道名称
#define RTT_CHANNEL_NAME_0              "Debug"
#define RTT_CHANNEL_NAME_1              "DATA"
...
static char rtt_buffer_down_DEBUG[RTT_BUFFER_SIZE_DOWN_DEBUG]; // 调试通道下行缓冲区
static char rtt_buffer_up_Data[RTT_BUFFER_SIZE_UP_Data];     // 数据通道上行缓冲区
static char rtt_buffer_down_Data[RTT_BUFFER_SIZE_DOWN_Data]; // 数据通道下行缓冲区
                               ...
                               const char* fmt, ...)
{
    char lineBuf[1024];
    char* out = lineBuf;
    char* end = lineBuf + sizeof(lineBuf) - 1; /* reserve one byte for NUL */

    {
        /* define as block-local function-like macro to keep header-only */
#define RTT_APPEND_NUM(_outp, _end, _num) do { \
            size_t _avail = (size_t)((_end) - *(_outp)); \
            if (_avail > 0) { \
                int _nw = snprintf(*(_outp), _avail, "%s%s%s", RTT_CTRL_TEXT_BLUE, (_num), RTT_CTRL_TEXT_WHITE); \
                if (_nw > 0) { if ((size_t)_nw > _avail) _nw = (int)_avail; *(_outp) += _nw; } \
            } \
        } while(0)
    ...
    /* 时间戳(白) + 级别(彩色) */
    {
        ...
        int w = 0;
        if (avail > 0) w = snprintf(out, avail, "%s[%lu]%s[%s]%s ",
                                     RTT_CTRL_TEXT_WHITE,
                                     (unsigned long)HAL_GetTick(),
                                     levelColor, level,
                                     RTT_CTRL_TEXT_WHITE);
        if (w < 0) w = 0;
        else if ((size_t)w > avail) w = (int)avail;
        out += w;
    }
    ...
    while (*p && out < end) {
        if (*p == '%') {
            ++p;
            /* 收集完整的格式说明符（包含标志、宽度、精度、长度等），以支持像 %03d 这样的格式 */
            char fmtSpec[32]; int fi = 0;
            fmtSpec[fi++] = '%';
            bool longFlag = false;
            while (*p && fi < (int)sizeof(fmtSpec) - 1) {
                /* 如果遇到转换说明符，结束收集 */
                if (*p == '%' || *p == 'c' || *p == 's' || *p == 'd' || *p == 'u' || *p == 'x' || *p == 'X' || *p == 'f' || *p == 'V' || *p == 'F') {
                    fmtSpec[fi++] = *p; /* include conv char */
                    fmtSpec[fi] = '\0';
                    break;
                }
            ...
            if (fi == 1) break; /* 意外结束 */
            char conv = fmtSpec[fi - 1];
            switch (conv) {
            case '%': *out++ = '%'; break;
            case 'c': {
                int v = va_arg(args, int);
                char tmp[16]; int n = snprintf(tmp, sizeof(tmp), fmtSpec, v);
                if (n > 0) { size_t copy = (size_t)n; if (out + copy > end) copy = (size_t)(end - out); memcpy(out, tmp, copy); out += copy; }
            } break;
            case 's': {
                const char* s = va_arg(args, const char*);
                if (!s) s = "(null)";
                char tmp[512]; int n = snprintf(tmp, sizeof(tmp), fmtSpec, s);
                if (n > 0) { size_t copy = (size_t)n; if (out + copy > end) copy = (size_t)(end - out); memcpy(out, tmp, copy); out += copy; }
            } break;
            case 'd': {
                if (longFlag) {
                    long v = va_arg(args, long);
                    char num[64]; snprintf(num, sizeof(num), fmtSpec, v);
                    RTT_APPEND_NUM(&out, end, num);
                } else {
                    int v = va_arg(args, int);
                    char num[64]; snprintf(num, sizeof(num), fmtSpec, v);
                    RTT_APPEND_NUM(&out, end, num);
            ...
            case 'f': {
                ...
                char num[96]; snprintf(num, sizeof(num), fmtSpec, dv);
                RTT_APPEND_NUM(&out, end, num);
            } break;
            case 'V': {
            ...
            case 'F': {
                double dv = va_arg(args, double);
                float fv = (float)dv;
                union { float f; uint32_t u; } convu; convu.f = fv;
                char bin[34]; bin[0] = '0'; bin[1] = 'b';
                for (int i = 0; i < 32; ++i) bin[2 + i] = (convu.u & (1u << (31 - i))) ? '1' : '0';
                bin[33] = 0;
                char num[96];
                snprintf(num, sizeof(num), "float=%f hex=0x%08lX bin=%s",
                         (double)fv, (unsigned long)convu.u, bin);
                const char* s = num;
                while (*s && out < end) {
                    if (*s >= '0' && *s <= '9') {
                        const char* st = s;
            ...
            default:
                /* 未识别：原样输出格式字符串 */
                for (int i = 0; i < fi && out < end; ++i) *out++ = fmtSpec[i];
                break;
            }
            /* 已消费转换字符，继续循环（p 指向转换字符位置，需要前移一位） */
            if (*p) ++p;
        } else {
            /* 直接复制常规字符；不对文本中的数字着色 */
            *out++ = *p++;
    ...
    /* 复位并换行，保证 NUL 终止 */
    {
        size_t avail = (size_t)(end - out);
        if (avail > 0) {
            int w = snprintf(out, avail, "%s\r\n", RTT_CTRL_RESET);
            if (w < 0) w = 0; else if ((size_t)w > avail) w = (int)avail;
            out += w;
        }
        if (out <= end) *out = '\0'; else *end = '\0';
    }

    SEGGER_RTT_WriteString(ch, lineBuf);

    /* cleanup macro to avoid leaking into other code */
#undef RTT_APPEND_NUM
}

// 使用: levelColor 传 RTT_CTRL_TEXT_RED/YELLOW/GREEN; 普通文本/数字自动处理
#define RTT_LOG_ERROR(fmt, ...) RTT_LogCore(RTT_CHANNEL_DEBUG, RTT_CTRL_TEXT_RED,    "ERROR", fmt, ##__VA_ARGS__)
#define RTT_LOG_WARN(fmt, ...)  RTT_LogCore(RTT_CHANNEL_DEBUG, RTT_CTRL_TEXT_YELLOW, "WARN",  fmt, ##__VA_ARGS__)
#define RTT_LOG_INFO(fmt, ...)  RTT_LogCore(RTT_CHANNEL_DEBUG, RTT_CTRL_TEXT_GREEN,  "INFO",  fmt, ##__VA_ARGS__)

#define RTT_LOG_DATA(fmt, ...)  RTT_LogCore(RTT_CHANNEL_DATA, RTT_CTRL_TEXT_GREEN,  "DATA",  fmt, ##__VA_ARGS__)
...
{
    // 配置通道0 - 调试
    SEGGER_RTT_ConfigUpBuffer(RTT_CHANNEL_DEBUG, RTT_CHANNEL_NAME_0,
                              rtt_buffer_up_DEBUG, RTT_BUFFER_SIZE_UP_DEBUG, RTT_MODE_DEFAULT);
    SEGGER_RTT_ConfigDownBuffer(RTT_CHANNEL_DEBUG, RTT_CHANNEL_NAME_0,
                                rtt_buffer_down_DEBUG, RTT_BUFFER_SIZE_DOWN_DEBUG, RTT_MODE_DEFAULT);

    // 配置通道1 - 数据
    SEGGER_RTT_ConfigUpBuffer(RTT_CHANNEL_DATA, RTT_CHANNEL_NAME_1,
                              rtt_buffer_up_Data, RTT_BUFFER_SIZE_UP_Data, RTT_MODE_DEFAULT);
    SEGGER_RTT_ConfigDownBuffer(RTT_CHANNEL_DATA, RTT_CHANNEL_NAME_1,
                                rtt_buffer_down_Data, RTT_BUFFER_SIZE_DOWN_Data, RTT_MODE_DEFAULT);
}
...
static inline void RTT_PrintChannelInfo(void)
{
    ...
    RTT_LOG_INFO("Channel 0 (%s): UP=%d bytes, DOWN=%d bytes\n",
             RTT_CHANNEL_NAME_0, RTT_BUFFER_SIZE_UP_DEBUG, RTT_BUFFER_SIZE_DOWN_DEBUG);
    RTT_LOG_INFO("Channel 1 (%s): UP=%d bytes\n",
             RTT_CHANNEL_NAME_1, RTT_BUFFER_SIZE_UP_Data);
