...
void CSystemCore::UpdateHandler_() {
    ...
    if (pcontroller_) {
        ...
        SysControllerLink.controllerInfo.arm.pitch1     = pcontroller_->ControllerInfo.posit_pitch1 - CONTROLLER_PITCH1_ZERO_OFFSET;//其实不是偏移而是机器人那边做了控制器模式下臂的限幅
        ...
        */
        if (SysControllerLink.robotInfo.robot_init_ok) {
            ...
            constexpr float i36v3_KT_RAW_TO_TORQUE = (33.0f / 2048.0f) / 0.175f;  // ≈ 0.00282 N·m/LSB
            constexpr float i8v3_KT_RAW_TO_TORQUE = (33.0f / 2048.0f) / 1.09f;  // ≈ 0.00282 N·m/LSB
            pcontroller_->ControllerCmd.fb_torque_yaw       = SysControllerLink.robotInfo.torque.yaw    * i8v3_KT_RAW_TO_TORQUE;
            pcontroller_->ControllerCmd.fb_torque_pitch1    = SysControllerLink.robotInfo.torque.pitch1 * i36v3_KT_RAW_TO_TORQUE;
            pcontroller_->ControllerCmd.fb_torque_pitch2    = SysControllerLink.robotInfo.torque.pitch2 * i36v3_KT_RAW_TO_TORQUE;
            // DM4310 MIT: 12bit原始值(0~4095) -> 物理力矩(-TAU_MAX ~ +TAU_MAX), TAU_MAX=10.0 N·m
            constexpr float DM_TAU_MAX = 10.0f;
            pcontroller_->ControllerCmd.fb_torque_roll = SysControllerLink.robotInfo.torque.roll / 4095.0f * (2.0f * DM_TAU_MAX) - DM_TAU_MAX;
            ...
            // pcontroller_->ControllerCmd.isfirstChange = false;
        }
    }
