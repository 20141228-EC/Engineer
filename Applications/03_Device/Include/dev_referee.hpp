...
namespace my_engineer 
{
	...
	enum class KeyCode		// Windows通用键值
	{
		A = 65,
		B = 66,
		C = 67,
...
public:
	...
	struct SCustomClientPkg {					// ID: ID_CUSTOM_CLIENT (0x306)
		uint8_t key_value1;						// 按键1键值
		uint8_t key_value2;						// 按键2键值
		uint16_t x_position : 12;				// 鼠标x轴位置
		uint16_t mouse_left : 4;				// 鼠标左键状态
		uint16_t y_position : 12;				// 鼠标y轴位置
